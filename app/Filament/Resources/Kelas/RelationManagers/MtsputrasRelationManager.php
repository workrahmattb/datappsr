...
class MtsputrasRelationManager extends RelationManager
{
    protected static string $relationship = 'mtsputras';

    protected static ?string $title = 'Siswa MTs Putra';

    protected static ?string $label = 'Siswa MTs Putra';

    protected static ?string $pluralLabel = 'Siswa MTs Putra';
        ...
        return $table
                        ...
                        Action::make('attachStudent')
                            ...
                            ->form([
                                Select::make('siswa_id')
                                    ...
                                    ->preload()
                                    ->options(
                                        \App\Models\Mtsputra::whereNull('kelas_id')
                                            ->orderBy('nama')
                                            ->pluck('nama', 'id')
                                    )
                                    ->required(),
                            ...
                            ->action(function (array $data, RelationManager $livewire) {
                                ...
                                // update siswa
                                \App\Models\Mtsputra::find($data['siswa_id'])
            ...
            ->recordActions([
                ViewAction::make('view')
                    ->label('Lihat')
                    ->icon('heroicon-o-eye')
                    ->url(fn ($record) => route('filament.admin.resources.mtsputras.edit', $record))
                    ->openUrlInNewTab(),
