...
namespace App\Filament\Resources\Maputras\Tables;
        ...
        return $table
            ...
            ->actions([
                ...
                ->action(function ($record) {
                    $pdf = Pdf::loadView('pdf.maputra', [
                        'record' => $record
                    ]);

                    return response()->streamDownload(function () use ($pdf) {
                        echo $pdf->output();
                    }, 'maputra-' . $record->id . '.pdf');
                }),
                ...
                Action::make('export')
                    ...
                    ->form([
                        Select::make('tahun_ajaran')
                            ...
                            ->options(
                                Maputra::query()
                                    ->whereNotNull('tahun_ajaran') // ← cegah null
                                    ->select('tahun_ajaran')
                                    ->distinct()
                    ...
                    ->action(function (array $data) {
                        ...
                        $tahunFile = preg_replace(
                            '/[^A-Za-z0-9\-]/',
                            '-',
                            str_replace('/', '-', $tahunAjaran)
                        );

                        $file = "maputra-{$tahunFile}-" . now()->format('YmdHis') . '.xlsx';

                        $rows = Maputra::where('tahun_ajaran', $tahunAjaran)->get();
