<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-zinc-200/80 p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-zinc-900 tracking-tight">Data Siswa MA Putra</h1>
                <p class="text-sm text-zinc-500 mt-1 font-medium">Kelola data siswa MA Putra yang terdaftar di sistem</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.maputras.export', ['tahun_ajaran' => $tahunAjaran]) }}" class="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg flex items-center shadow-sm hover:shadow transition-all duration-200 font-semibold text-sm">
                ...
                <a wire:navigate href="{{ route('admin.maputras.create') }}" class="bg-zinc-900 hover:bg-zinc-800 text-white px-5 py-2.5 rounded-lg flex items-center shadow-sm hover:shadow transition-all duration-200 font-semibold text-sm">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200/80">
                ...
                <tbody class="bg-white divide-y divide-zinc-200/80">
                    @forelse($maputras as $maputra)
                    <tr class="hover:bg-zinc-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <span class="w-5 text-xs font-bold text-zinc-400 text-center shrink-0">{{ $maputras->firstItem() + $loop->index }}</span>
                                @if($maputra->foto)
                                    <img src="{{ Storage::url($maputra->foto) }}" alt="Foto {{ $maputra->nama }}" class="w-9 h-9 rounded-full object-cover border border-zinc-200 shrink-0 ml-1.5">
                                @else
                                    <div class="w-9 h-9 bg-blue-100 border border-zinc-200 rounded-full flex items-center justify-center text-blue-600 font-bold text-sm shrink-0 ml-1.5">
                                        {{ strtoupper(substr($maputra->nama, 0, 1)) }}
                                    </div>
                                @endif
                                <div class="ml-2.5">
                                    <div class="text-sm font-bold text-zinc-900">{{ $maputra->nama }}</div>
                                    <div class="text-xs text-zinc-500 mt-0.5 font-medium">{{ $maputra->nik }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-sm text-zinc-700 font-medium">{{ $maputra->nisn ?? '-' }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-zinc-100 border border-zinc-200 text-zinc-800 text-xs font-semibold">
                                {{ $maputra->tahun_ajaran ?? '-' }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm text-zinc-900 font-medium">{{ \Carbon\Carbon::parse($maputra->tanggal_lahir)->translatedFormat('d F Y') }}</div>
                            <div class="text-xs text-zinc-500 mt-0.5">{{ $maputra->tempat_lahir }}</div>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a wire:navigate href="{{ route('admin.maputras.show', $maputra) }}" class="p-1.5 text-zinc-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Lihat">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a wire:navigate href="{{ route('admin.maputras.edit', $maputra) }}" class="p-1.5 text-zinc-500 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-sm font-bold text-zinc-900 mb-1">Belum ada data siswa</h3>
                                <p class="text-sm text-zinc-500">Data siswa MA Putra yang ditambahkan akan muncul di sini.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <flux:modal name="delete-confirm" class="max-w-md" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); margin: 0;">
        <div class="p-6">
            <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h3 class="text-lg font-bold text-zinc-900">Hapus Data Siswa MA Putra?</h3>
            ...
            <div class="bg-red-50 border border-red-100 rounded-lg p-4 mb-6">
                <p class="text-sm text-zinc-700">
                    Anda akan menghapus data siswa MA Putra atas nama
